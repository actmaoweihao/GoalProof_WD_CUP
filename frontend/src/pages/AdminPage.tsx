...
export function AdminPage() {
  ...
  async function submitResult() {
    try {
      assertMatchId();
      if (![homeScore, awayScore].every((score) => Number.isInteger(score) && score >= 0 && score <= 30)) {
  ...
  return (
    <section className="page-section">
      ...
      <div className="admin-grid">
        <section className="admin-card"><div className="eyebrow">CREATE MATCH</div><h2>创建演示比赛</h2><div className="form-grid"><label className="field"><span>主队</span><input value={homeTeam} onChange={(e) => setHomeTeam(e.target.value)} /></label><label className="field"><span>客队</span><input value={awayTeam} onChange={(e) => setAwayTeam(e.target.value)} /></label><label className="field"><span>承诺截止</span><input type="datetime-local" value={commitAt} onChange={(e) => setCommitAt(e.target.value)} /></label><label className="field"><span>开赛时间</span><input type="datetime-local" value={kickoffAt} onChange={(e) => setKickoffAt(e.target.value)} /></label><label className="field field-wide"><span>公开截止</span><input type="datetime-local" value={revealAt} onChange={(e) => setRevealAt(e.target.value)} /></label></div><button className="button button-primary button-wide" disabled={!roles[0] || isPending} onClick={createMatch}>创建比赛</button></section>
        <section className="admin-card"><div className="eyebrow">ORACLE RESULT</div><h2>提交最终赛果</h2><div className="form-grid"><label className="field field-wide"><span>比赛 ID</span><input type="number" min="1" value={matchId} onChange={(e) => setMatchId(Number(e.target.value))} /></label><label className="field"><span>主队比分</span><input type="number" min="0" max="30" value={homeScore} onChange={(e) => setHomeScore(Number(e.target.value))} /></label><label className="field"><span>客队比分</span><input type="number" min="0" max="30" value={awayScore} onChange={(e) => setAwayScore(Number(e.target.value))} /></label></div><button className="button button-primary button-wide" disabled={!roles[1] || isPending} onClick={submitResult}>提交不可修改的赛果</button><button className="button button-ghost button-wide" disabled={!roles[0] || isPending} onClick={cancelMatch}>取消这场比赛</button></section>
      </div>
